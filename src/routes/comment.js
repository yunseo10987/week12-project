...
router.post("/", async (req, res) => {
    const sql = "INSERT INTO backend.comment(content, post_idx, account_idx) VALUES($1, $2, $3)"
    ...
    try{
        ...
        await client.query(sql, [content, postIdx, req.session.idx])
        result.success = true
...
router.get("/", async (req, res) => {
    ...
    try{
        const comment = await client.query(sql, [postIdx, pageNumber])
        result.success = true
...
router.put("/:commentIdx", async (req, res) => {
    const sql = "UPDATE backend.comment SET content=$1 WHERE idx = $2 AND account_idx =$3"
...
router.put("/commentIdx/likes", async (req, res) => {
    ...
    try{
        ...
        const isLikers = await client.query('SELECT idx FROM backend.comment_likes WHERE comment_idx = $1 AND account_idx = $2', [commentIdx,req.session.idx])
        if(isLikers.rows){
            await client.query('DELETE FROM backend.comment_likes WHERE comment_idx = $1 AND account_idx = $2', [commentIdx, req.session.idx])
            await client.query('UPDATE backend.comment SET comment_likes = comment_likes - 1 WHERE idx = $1 AND account_idx = $2')

            result.message = "좋아요 취소"
        }else{
            await client.query('INSERT INTO backend.comment_likes(comment_idx, account_idx) VALUES($1, $2)', [commentIdx, req.session.idx])
            await client.query('UPDATE backend.comment SET comment_likes = comment_likes + 1 WHERE idx = $1 AND account_idx = $2')
