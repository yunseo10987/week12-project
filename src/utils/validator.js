...
module.exports = {
  ...
  duplicatedId: async function (id) {
    const sql = "SELECT id FROM backend.account WHERE id =$1";
    const account = await client.query(sql, [id]);
    console.log(account.rows);

    if (account.rows.length && account.rows[0].id != id) {
  ...
  // validator : function (arg, regex , message){
  //     if(arg == "" || arg == "undefined" || arg == null){
  //         throw new Error(message)
  //     }
  //     if(regex == "" || regex == "undefined" || regex == null){
  //         if(!regex.test(arg)){
  //             throw new Error(`${arg} is not validated`)
  //         }
  //     }
